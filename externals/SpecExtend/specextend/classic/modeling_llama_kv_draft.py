...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        if init:
            ...
            # print(colored(f'PAST kv_seq_len:{kv_seq_len}','magenta'))
            ...
            # Save past_key_position_ids for next iteration
        ...
        if not output_attentions:
            ...
            if init:
                # attn_weights = attn_weights[:, :, -1, :].detach().cpu()
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            current_output_attentions = output_attentions if idx == len(self.layers) - 1 else False
            ...
            if self.gradient_checkpointing and self.training:
                ...
                layer_outputs, past_key_position_ids = decoder_layer(
                    ...
                    position_ids=position_ids,
                    past_key_value=past_key_value,
                    output_attentions=current_output_attentions,
                    use_cache=use_cache,
                    return_kv=return_kv,
                    init=init,
            ...
            if use_cache:
                next_decoder_cache += (layer_outputs[2 if current_output_attentions else 1],)

            if current_output_attentions:
