...
class Tree:
    ...
    def add(self,logits):
        # expand top 50 nodes for all the 50 nodes in the previous layer
        ...
        # flatten and choose the top 50 of the 2500 expanded nodes
        ...
        # save the top 50 to weight_matrix and input_ids matrix
        ...
        pvals = global_top_logits.tolist()
        # print(colored("Prefix probabilities:\n" + " ".join(f"{p:.3f}" for p in pvals), 'magenta'))

        # Create mask for the new nodes
    ...
    def update(self,logits):
        ...
        # otherwise add a layer of 50 nodes
        outputs=self.add(logits)
        top_weights,top_index=torch.topk(self.weight_matrix[:self.depth-1].view(-1),k=self.nnodes,dim=-1)

        # sum of all probabilities of the tree (= total expected acceptance length)
        weight=top_weights.sum()
        # print(colored(f'prev weights: {self.weight:.3f} => current weights: {weight:.3f}', 'red'))
        if weight-self.weight>self.threshold and self.depth<self.max_depth:
            # continue drafting
            self.weight=weight
        else:
            # finish drafting => choose the best 50 nodes from all layers combined
