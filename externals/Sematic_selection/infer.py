...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=0,
        help=(
            "Truncate each document to this many tokens before selection and "
            "generation (0 = no limit; use on T4 smoke runs for govreport)."
        ),
    )
    parser.add_argument(
        "--save-selected-text",
...
def main() -> None:
    ...
    if args.max_input_tokens and args.max_input_tokens > 0:
        # Cap very long documents (e.g. govreport) so the dense "full" reference
        # run and every selector fit a T4 16GB in smoke runs.
        for ex in examples:
            enc = tokenizer(
                ex.document, truncation=True, max_length=args.max_input_tokens
            )
            ex.document = tokenizer.decode(
