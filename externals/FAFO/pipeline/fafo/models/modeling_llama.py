...
try:
    ...
    # Removed from Transformers 5; this guard is only used to mark one helper
    # as an FX leaf on older versions.
...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig, layer_idx: Optional[int] = None):
        ...
        if self.rope_theta is None:
            ...
            # LlamaRotaryEmbedding_L31 delegates to Transformers' RoPE
            # initializer, which still expects the legacy top-level field.
...
LLAMA_INPUTS_DOCSTRING = r"""
    Args:
        ...
        past_key_values (`tuple(tuple(torch.FloatTensor))`, *optional*, returned when `_use_cache=True` is passed or when `config._use_cache=True`):
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def LlamaModeljforward(
        ...
        r"""
        Manipulate the attention mask & feed-forward for LookAHead Decoding + Verification
        """
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def jforward_multilevel(
        ...
        if labels is None and not swap_axis_for_flash and self.config.pretraining_tp <= 1:
            assert fill_level != -1
            ...
            ret.out_logits = self.lm_head(hidden_states[:, prefill_size - 1:prefill_size, :])[:, 0, :]  #decode logits
            if lguess > 0:
                llookahead = sum(level_sizes)
                start = size - llookahead - lguess
                end = size - llookahead
                ret.inp_logits = self.lm_head(hidden_states[:, -window:, :])       #lookahead branch logits
                ret.guess_logits = self.lm_head(hidden_states[:, start:end, :])    #verification branch logits
            else:
                ret.inp_logits = self.lm_head(hidden_states[:, size - window:size, :])  #lookahead branch logits
            return ret

        # ---- full projection path (training / flash axis-swap) ----
        if self.config.pretraining_tp > 1:
            lm_head_slices = self.lm_head.weight.split(self.vocab_size // self.config.pretraining_tp, dim=0)
            logits = [F.linear(hidden_states, lm_head_slices[i]) for i in range(self.config.pretraining_tp)]
            logits = torch.cat(logits, dim=-1)
