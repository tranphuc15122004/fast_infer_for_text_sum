...
def compress_fastkv(model, args): 
    layers = len(model.model.layers)

    for i in range(layers):
        model.model.layers[i].self_attn.kv_cluster.window_size = args.window_size[i]
        model.model.layers[i].self_attn.kv_cluster.kernel_size = args.kernel_size[i]
        model.model.layers[i].self_attn.kv_cluster.pooling = args.pooling
        model.model.layers[i].self_attn.kv_cluster.max_capacity_prompt = args.max_capacity_prompts
        model.model.layers[i].self_attn.kv_cluster.tsp_length = args.tsp_len
        model.model.layers[i].self_attn.kv_cluster.tsp_rate = args.tsp_rate
        model.model.layers[i].self_attn.kv_cluster.eviction_mode = args.eviction_mode
        ...
        if i <= args.tsp_idx:
            model.model.layers[i].self_attn.kv_cluster.retain_rate = args.retain_rate
        else:
            model.model.layers[i].self_attn.kv_cluster.retain_rate = args.retain_rate/args.tsp_rate
            
            
class FastKVCluster():
    def __init__(self, window_size=8, max_capacity_prompt=512, kernel_size=7, pooling='avgpool', 
                 tsp_layer=False, tsp_length=2048, tsp_rate=0.25, retain_rate=0.25, eviction_mode="constant"):
        ...
        self.tsp_rate = tsp_rate

    def reset(self, window_size=8, max_capacity_prompt=512, kernel_size=7, pooling='avgpool', tsp_layer=False, 
              tsp_length=2048, tsp_rate=0.25, retain_rate=0.25, eviction_mode="constant"):
        self.window_size = window_size
        self.max_capacity_prompt = max_capacity_prompt
        assert self.max_capacity_prompt - self.window_size > 0
        self.kernel_size = kernel_size
    ...
    def update_kv(self, key_states, query_states, value_states, attention_mask, num_key_value_groups, layer_idx):
        ...
        if q_len < self.max_capacity_prompt:
            ...
            if self.tsp_layer and self.eviction_mode == "proportional":
                self.tsp_length = int(q_len * self.tsp_rate)
