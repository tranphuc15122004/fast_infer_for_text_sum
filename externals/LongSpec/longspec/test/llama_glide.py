...
class LlamaGlide(LlamaForCausalLM):
    def __init__(self, config, target_model_path, glide_path=None):
        ...
        if glide_path is None:
            self.glide = LlamaGlideDecoderLayer(config)
        else:
            self.glide = LlamaGlideDecoderLayer.from_pretrained(glide_path, torch_dtype=torch.float16, device_map="auto")
    ...
    def tree_spec_generate(self, input_ids, prompt_length, tree_shape=None, max_gen_len=64, eos_id=151645, temperature=0.0):
        ...
        self.glide.set_max_gen_len(max_gen_len + 256)
