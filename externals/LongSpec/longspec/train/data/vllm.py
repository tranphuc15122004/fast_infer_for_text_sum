    ...
    def __call__(self, prompt: str) -> str:
        ...
        while True:
            if "completions" not in self.api_url:
                ...
                messages = []
                if "system_prompt" in self.kwargs:
                    messages.append({"role": "system", "content": self.kwargs.pop("system_prompt")})
                messages.append({"role": "user", "content": prompt})

                response = post_http_request(messages=messages,
                                             api_url=self.api_url,
                                             n=self.n,
                                             max_tokens=max_tokens,
                                             temperature=self.temperature,
                ...
                if response.status_code != 200:
                    ...
                    for item in json.loads(response.content)["choices"]:
                        outputs.append(item["message"]["content"].replace(prompt, ""))
                    if len(outputs) == 1:
                        response = outputs[0]
                    else:
                        response = outputs
