...
class GlideAttention(nn.Module):
    ...
    def prefill(
        ...
        cos, sin = position_embeddings
        query_states, key_states = apply_rotary_pos_emb(query_states, key_states, cos, sin, unsqueeze_dim=2)
        self.K_Cache = query_states.new_zeros((bsz, q_len + self.max_len + 128, self.num_key_value_heads, self.head_dim))
        self.V_Cache = query_states.new_zeros((bsz, q_len + self.max_len + 128, self.num_key_value_heads, self.head_dim))
...
class LlamaGlideDecoderLayer(PreTrainedModel):
    config_class = LlamaConfig
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = ["GlideAttention", "LlamaMLP", "LlamaRMSNorm"]
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.post_self_attention_layernorm = LlamaRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
...
class LlamaGlide(PretrainedModelParallelPreSplitMixin, LlamaForCausalLM):
    def __init__(self, config, target_model_path, glide_path=None):
        super().__init__(config)
        model = LlamaForCausalLM.from_pretrained(target_model_path, torch_dtype=torch.float16, device_map="auto")
        self.model = model.model
        self.lm_head = model.lm_head
        if glide_path is None:
            self.draft_model = LlamaGlideDecoderLayer(config)
        else:
            self.draft_model = LlamaGlideDecoderLayer.from_pretrained(glide_path, torch_dtype=torch.float16, device_map="auto")
    ...
    def vanilla_torch_generate(self, input_ids, prompt_length, max_gen_len=64, eos_id=151645):
        assert input_ids != None, "please give the input"
        bsz = input_ids.size(0)
        output_ids = input_ids.new_zeros((bsz, max_gen_len))
        ...
        cache_lens = input_ids.new_zeros((bsz)).int()
        hidden_states = self.model.forward(input_ids, exec_type="prefill_torch").last_hidden_state
        input_len = prompt_length
        output_ids[:, 0] = self.lm_head(hidden_states[range(bsz), input_len-1, :]).argmax(dim=-1)
        cache_lens += input_len
        num = 0
        ...
        for _ in range(1, max_gen_len):
            input_ids = output_ids[range(bsz), cache_lens - input_len].view(bsz, -1)
            hidden_states = self.model.forward(input_ids, cache_lens=cache_lens.clone(), exec_type="decoding_torch").last_hidden_state
            llm_output = self.lm_head(hidden_states[:, -1, :]).argmax(dim=-1)
            cache_lens += 1
            num += bsz
            output_ids[range(bsz), cache_lens - input_len] = llm_output.view(-1)
    ...
    def magicdec_generate(self, input_ids, prompt_length, gamma=4, max_gen_len=64, eos_id=151645, temperature=0.0):
        assert input_ids != None, "please give the input"
        bsz = input_ids.size(0)
        output_ids = input_ids.new_zeros((bsz, max_gen_len + gamma))
        spec_mask = input_ids.new_zeros((bsz, max_gen_len + gamma))
        ...
        cache_lens = input_ids.new_zeros((bsz)).int()
        hidden_states = self.model.forward(input_ids, exec_type="magicdec_prefill")["last_hidden_state"]
        input_len = prompt_length
        logits = self.lm_head(hidden_states[range(bsz), input_len-1, :])
        output_ids[:, 0] = logits.argmax(dim=-1)
        cache_lens += input_len
        ...
        for out_index in range(1, max_gen_len):
            ...
            for spec_steps in range(0, gamma):
                ...
                if spec_steps == 0:
                    if double_flag:
                        magicdec_input_ids = next_spec_start_token[:, 0:2]
                        position_ids = torch.arange(0, 2)[None, :].to(input_ids.device) + draft_cache_lens[:, None]
                        position_embeddings = self.model.rotary_emb(hidden_states, position_ids)
                    else:
                        magicdec_input_ids = next_spec_start_token[:, 0, None]
                        position_ids = draft_cache_lens[:, None]
                        position_embeddings = self.model.rotary_emb(hidden_states, position_ids)            

                else:
                    magicdec_input_ids = spec_buffer[:, spec_steps, None]
                    position_ids = draft_cache_lens[:, None]
                    position_embeddings = self.model.rotary_emb(hidden_states, position_ids)

                magicdec_cache_lens = (draft_cache_lens.clone() - input_len + 1024 + 32).to(torch.int32)
                hidden_states = self.model.forward(
                    magicdec_input_ids, 
                    position_embeddings=position_embeddings,
                    cache_lens=magicdec_cache_lens, 
                    exec_type="magicdec_decoding",
                )["last_hidden_state"]
    ...
    def tree_spec_generate(self, input_ids, prompt_length, tree_shape=None, max_gen_len=64, eos_id=151645, temperature=0.0):
        ...
        bsz = input_ids.size(0)
        output_ids = input_ids.new_zeros((bsz, max_gen_len)).fill_(eos_id)
        ...
        for out_index in range(1, max_gen_len):
            ...
            if temperature > 0:
                ...
                acc_ids, acc_num = self.verify_stochastic(
                    ...
                    p_ssm=spec_logits,        # bsz * f_seq, speculative llm logits
