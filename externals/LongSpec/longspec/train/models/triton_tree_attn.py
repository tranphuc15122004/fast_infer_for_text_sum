...
if __name__ == "__main__":
    ...
    k = torch.randn([B, 8, M, K], device=device, dtype=dtype)
    v = torch.randn([B, 8, M, K], device=device, dtype=dtype)

    mask = torch.zeros((B, M, M), device=device, dtype=dtype).normal_()
    mask_id = mask.clone()
