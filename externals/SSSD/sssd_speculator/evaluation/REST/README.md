> **NOTICE — Modified third-party code.** This directory contains a modified
> copy of REST, originally distributed at
> <https://github.com/FasterDecoding/REST> under the Apache License 2.0
> (Copyright © The REST authors; see [`LICENSE`](LICENSE)). It is redistributed
> here under the same license.
>
> **Scope of changes vs. upstream:**
>
> - [`DraftRetriever/src/lib.rs`](DraftRetriever/src/lib.rs) — `Reader::new()`
>   now auto-detects u16 vs. u32 token encoding so the Reader can load both
>   original REST datastores and SSSD datastores built for models with
>   vocabulary sizes greater than 65536 tokens. The Writer is unchanged; SSSD
>   datastores must be built with the SSSD Writer, which prepends a 0-valued
>   flag to signal the u32 format (this is the mechanism described in the
>   upstream [Other Models and Datastore](#other-models-and-datastore) section).
> - [`DraftRetriever_adapted/`](DraftRetriever_adapted/) — a second copy of
>   DraftRetriever with further changes to produce SSSD/SGLang-compatible
>   output from `Reader::search()`. See that folder's `src/lib.rs` header and
>   the root-level [`THIRD_PARTY_NOTICES.md`](../../THIRD_PARTY_NOTICES.md) for
>   the full list of changes.
> - No other files in this directory were modified.
>
> The `DraftRetriever/` and `DraftRetriever_adapted/` subdirectories are
> covered by their own MIT `LICENSE` files rather than by this directory's
> Apache 2.0 `LICENSE`.
