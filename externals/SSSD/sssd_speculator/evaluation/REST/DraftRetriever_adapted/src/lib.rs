// NOTICE: This file has been modified from its original version distributed as
// part of https://github.com/FasterDecoding/REST (MIT License, Copyright (c)
// 2022 Gal Ben David; 2023 Zhenyu He, Zexuan Zhong, Tianle Cai). It starts
// from the large-vocabulary-fixed `DraftRetriever/src/lib.rs` in the sibling
// folder and adapts it to the SSSD/SGLang speculative decoding interface.
//
// Modifications (see THIRD_PARTY_NOTICES.md at the repository root for details):
//   - PyO3 module renamed from `draftretriever` to `draftretriever_adapted` so
//     both variants can be installed and imported side by side.
//   - `Reader::search()` now returns a 3-tuple (flat_candidates, trie_attn_mask,
//     depths) instead of the original 5-tuple (paths, draft_attn_mask,
//     tree_indices, draft_position_ids, retrieve_indices).
//   - `pad_path()` and `generate_draft_buffers()` were removed.
//   - A new `FinalTrie` / `FinalTrieNode` data structure performs depth-first
//     traversal with deeper branches first, producing the flat candidate list,
//     attention mask, and depth vector returned by `Reader::search()`.
//   - The u16/u32 token-format auto-detection in `Reader::new()` is inherited
//     from the large-vocabulary-fixed `DraftRetriever`.
//
