"""
python plot_speedups_from_json.py \
  --root final_runs/1x4090_8B/evaluation \
  --out final_runs/1x4090_8B/analysis \
...
def collect_throughputs(root: str) -> Dict[str, Dict[int, Dict[str, float]]]:
    ...
    for dataset in sorted(os.listdir(root)):
        ...
        for fn in os.listdir(ds_dir):
            ...
            if "output_throughput" not in js:
                print(f"Missing 'output_throughput' in {fpath}, skipping.")
                continue

            tp = js["output_throughput"]
            if not isinstance(tp, (int, float)):
                print(f"'output_throughput' not numeric in {fpath}, skipping.")
                continue

            data[dataset][bs][method] = float(tp)
...
def plot_latency_vs_throughput(
    ...
    eagle_colors = {
        ...
        for m in ("EAGLE2", "EAGLE3", "LLama-3.2-1B")
