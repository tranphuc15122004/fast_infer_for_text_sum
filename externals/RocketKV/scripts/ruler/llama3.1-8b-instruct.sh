...
for max_seq_length in '16000' '32000' '64000' '96000'; do
  if [ $method == "baseline" ]; then
    for dataset in niah_single_1 niah_single_2 niah_single_3 niah_multikey_1 niah_multikey_2 niah_multikey_3 niah_multivalue niah_multiquery vt cwe fwe qa_1 qa_2; do
      ...
      --eval_config_dir config/eval_config/${task}.json \
      --output_folder_dir ${output_dir_root}/${task}_${max_seq_length}/${method}/${model}/
    done
  else
    token_budget=$3
    for dataset in niah_single_1 niah_single_2 niah_single_3 niah_multikey_1 niah_multikey_2 niah_multikey_3 niah_multivalue niah_multiquery vt cwe fwe qa_1 qa_2; do
      ...
      --pipeline_config_dir config/pipeline_config/${task}/${model}.json \
      --eval_config_dir config/eval_config/${task}.json \
      --output_folder_dir ${output_dir_root}/${task}_${max_seq_length}/${method}/${token_budget}/${model}/
    done
  fi
done
