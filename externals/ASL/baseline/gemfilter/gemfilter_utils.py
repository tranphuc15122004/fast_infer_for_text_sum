...
def find_context(self, query_states, key_states, print_idx_dis=False,_is_asl=False):
    b, h, n, d = key_states.shape
    if (self.indecies is None and self.layer_idx == self.select_layer_idx) or _is_asl:
        assert b == 1
...
def get_layer_context_asl(model, tokenizer,input_ids,print_context=False,use_asl_score=True):
def get_layer_context_asl(model, tokenizer,input_ids,print_context=False,use_asl_score=True):
    ...
    _ = model(
        **model_inputs
    ) #Best selection layer is set in this calc stage.
    
    past_key_values = model.model.past_key_values
    
    layer_idx = past_key_values.gemfilter_select_layer
    model.gemfilter_select_layer = layer_idx #saving for analysis
    if not use_asl_score:
        #default: Gemfilter
        select_idx = model.model.layers[layer_idx].self_attn.indecies[0, 0, :]
    else:
        #option: use same select token indices as ASL/FastKV
        if past_key_values.select_idx!=None:
            select_idx = past_key_values.select_idx.view(-1)
        else:
            mean_socre = past_key_values.prefilling_score_cache[layer_idx] #[total_len]
            seqlen= mean_socre.shape[-1]
            k=past_key_values.select_k - past_key_values.window_size if seqlen > past_key_values.select_k - past_key_values.window_size else seqlen 
            topk_idx = torch.topk(mean_socre, k=k,dim=-1,largest=False)[1]
            window_indices = torch.arange(mean_socre.shape[-1], mean_socre.shape[-1] + past_key_values.window_size, device=input_ids.device) #attn_cacheはwindow_indciesを除いて作成されているのでここで追加する必要がある
            select_idx = torch.cat([topk_idx[-1],window_indices]).unique().view(-1)
    idx = select_idx
    values, _ = torch.sort(idx)
    values = values.to(input_ids)
    # print(F"{input_ids.shape=}, {values.shape=}")
    new_input_ids = input_ids[0].gather(0, values)
    if print_context:
        print(tokenizer.decode(new_input_ids))
    return new_input_ids.unsqueeze(0)
...
def set_topk(model, topk, mode='gemfilter'):
    # print(F"gemfilter: set_topk={topk}")
...
@torch.no_grad()
def gemfilter_generate(model, tokenizer, pred_token_idx, past_key_values, max_gen_len,init_time):
    ...
    for i in range(max_gen_len):
        ...
        if i==0:
            prefill_time = time.time()- init_time
        past_key_values = outputs.past_key_values
        pred_token_idx = outputs.logits[:, -1, :].argmax(dim=-1).unsqueeze(1)
        if pred_token_idx == tokenizer.eos_token_id:
            break
        generated_ids.append(pred_token_idx.item())
    return generated_ids,prefill_time
...
def gemfilter_generate_selection(input_ids, attn_mask, model, tokenizer, max_gen_len=50, select_layer_idx=None, print_context=False):
    init_time =time.time()
    use_asl = bool(model.cache_kwargs.layer_th)
    model.gemfilter_select_layer = select_layer_idx if not use_asl else None #add for time measuring 
    if not use_asl:
        ...
        #これがasl verのgemfilterとなる
        set_select_mode(model, True)
        new_input_ids = get_layer_context_asl(model, tokenizer,input_ids,print_context)
        set_select_mode(model, False)
    ...
    output_ids,prefill_time = gemfilter_generate(model, tokenizer, pred_token_idx, past_key_values, max_gen_len=max_gen_len,init_time=init_time)
    output_time = time.time()- init_time
    response = tokenizer.decode(output_ids, skip_special_tokens=True).strip()
    output_length=len(output_ids)
    return response,prefill_time,output_time,output_length
