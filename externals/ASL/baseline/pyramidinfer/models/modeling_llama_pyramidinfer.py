...
def apply_rotary_pos_emb(q, k, cos, sin, position_ids=None, selected_position_ids=None, unsqueeze_dim=1):
    ...
    # selected_position_ids [bs, selected_seq_len]
    # gather cos and sin from selected_position_ids
    if selected_position_ids is not None:
        selected_position_ids = selected_position_ids.unsqueeze(-1).expand(-1, -1, cos.shape[-1])
        cos = cos.gather(dim=1, index=selected_position_ids)
        sin = sin.gather(dim=1, index=selected_position_ids)
...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        selected_position_ids: Optional[int] = None, # original position ids for compressed key value states
        ...
        **kwargs,
        ...
        query_states, key_states = apply_rotary_pos_emb(query_states, key_states, cos, sin, position_ids, selected_position_ids)

        if past_key_value is not None:
            # sin and cos are specific to RoPE models; cache_position needed for the static cache
            cache_kwargs = {"sin": sin, "cos": cos, "cache_position": cache_position}
            past_key_value.prefilling_score_update(query_states,key_states,value_states,self.layer_idx,just_for_analysis=True) #for time alaysis
    ...
    def forward(
        ...
        **kwargs,
        ...
        hidden_states, self_attn_weights, present_key_value = self.self_attn(
            ...
            output_attentions=output_attentions,
            use_cache=use_cache,
            selected_position_ids=selected_position_ids, # original position ids for compressed key value states
            cache_position=cache_position,
            position_embeddings=position_embeddings,
            **kwargs,
        )
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        # initialization for pyramidinfer
        if use_cache:
            seq_length_with_past = inputs_embeds.shape[1] + past_key_values.get_seq_length() if past_key_values is not None else 0
            selected_position_ids = torch.arange(seq_length_with_past, device=position_ids.device).unsqueeze(0).expand(input_ids.size(0), -1)
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if use_cache:
                if past_key_values.get_seq_length(0) <= inputs_embeds.shape[1]:
                    pyramid_causal_mask = causal_mask[:, :, -hidden_states.shape[1]:, -hidden_states.shape[1]:]
                else:
                    pyramid_causal_mask = causal_mask[:, :, -(past_key_values.get_seq_length(idx) + 1):, -(past_key_values.get_seq_length(idx) + 1):]
            ...
            if self.gradient_checkpointing and self.training:
                ...
                layer_outputs = decoder_layer(
                    hidden_states,
                    attention_mask=pyramid_causal_mask if use_cache else causal_mask,
                    position_ids=position_ids,
                    past_key_value=past_key_values,
                    output_attentions=True if use_cache else output_attentions,
                    use_cache=use_cache,
                    cache_position=cache_position,
                    position_embeddings=position_embeddings,
                    selected_position_ids=selected_position_ids if past_key_values.get_seq_length(0) <= inputs_embeds.shape[1] and use_cache else None,
                )
            ...
            if use_cache:
                ...
                with torch.no_grad():
                    ...
                    if past_key_values.get_seq_length(0) <= inputs_embeds.shape[1]:
                        ...
                        # print(F"prefill: {len(recent_attn_weights)=}, {past_kv_seq_lens=}")
                    else:
                        ### generation stage ###
                        # print(F"generate: {len(recent_attn_weights)=}, {past_kv_seq_lens=}")
                        attn_weights = layer_outputs[1].mean(dim=1) # average over attention heads
        ...
        return BaseModelOutputWithPastForPMT(
...
class LlamaForCausalLM(LlamaPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **loss_kwargs,
        ...
        # print(F"{len(recent_attn_weights)=}")
        outputs = self.model(
            input_ids=input_ids,
            attention_mask=attention_mask,
            position_ids=position_ids,
        ...
        return CausalLMOutputWithPastForPMT(
            loss=loss,
            logits=logits,
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        self.model.past_key_values = past_key_values #saving用に追加
        return model_inputs
