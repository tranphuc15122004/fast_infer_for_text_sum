...
def plot_needle_viz(
    ...
    def get_context_size(x, is_128k: bool = True):
        if is_128k:
    ...
    # plt.title(
    #     f"Needle in A Haystack {model_name} {get_context_size(max_context)} Context" #修正する
    # )  # Adds a title

    model_names = mode.split("_")
    ...
    # xtick_labels = [get_context_size(x, max_context == "128K") for x in xtick_labels]
    xtick_labels = [get_context_size(x, True) for x in xtick_labels]
    # xtick_labels = [get_context_size(x, max_context == "128K") for x in xtick_labels]
    xtick_labels = [get_context_size(x, True) for x in xtick_labels]
    ...
    save_path = os.path.join(
        output_path,
        f"./{model_name}/needle_viz_{mode}_{min_context_str}_{max_context_str}.pdf",
    )
