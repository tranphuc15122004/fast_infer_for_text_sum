...
class TrajectoryRecord:
    ...
    def from_json(cls, raw: dict) -> "TrajectoryRecord":
        allowed = {
            ...
            "target_features", "source_memory", "metadata",
        }
...
class TrajectoryCache:
    ...
    def _torch_record_payload(record: TrajectoryRecord) -> dict:
        raw = asdict(record)
        for field_name in ("target_logits", "target_features", "source_memory"):
...
def build_stage1_batch(
    ...
    masked = torch.full((len(rows), kd), int(mask_token_id), dtype=torch.long, device=device)
    targets = torch.zeros((len(rows), kd), dtype=torch.long, device=device)
    valid = torch.zeros((len(rows), kd), dtype=torch.bool, device=device)
    for row, record in enumerate(rows):
        ...
        if values:
            targets[row, : len(values)] = torch.tensor(values, dtype=torch.long, device=device)
...
def anchor_position_offsets(
    ...
    drafter therefore sees the same absolute position it will see at serving:
    prompt length plus the number of committed target tokens before the
    anchor.  Metadata is preferred because it preserves the exact tokenized
    prompt length used by Stage 0; old caches fall back to ``source_ids``.
    """
    rows = list(records)
    ...
    for record, anchor in zip(rows, anchors):
        ...
        if (
            ...
            and len(stored) == len(record.anchors)
            and int(anchor) in record.anchors
        ):
            anchor_slot = record.anchors.index(int(anchor))
            if anchor_slot < len(stored):
                offsets.append(max(0, int(stored[anchor_slot])))
                continue
        offsets.append(max(0, context_length + int(anchor)))
...
class SyncSpecTrainer:
    ...
    @staticmethod
    def _batch_indices(total: int, batch_size: int | None, step: int) -> list[int]:
        """Return deterministic cyclic row indices for one optimizer step."""
        total = int(total)
        if total <= 0:
    ...
    def fit_diffusion(
        ...
        rows = list(records)
        if not rows:
            raise ValueError("diffusion training requires at least one trajectory record")
        ...
        for step in range(int(steps)):
            indices = self._batch_indices(len(rows), batch_size, step)
            batch_rows = [rows[index] for index in indices]
            anchor_indices = sample_anchor_indices(batch_rows, self._anchor_generator)
            ...
            with self._autocast():
                ...
                last_tensor = diffusion_loss(
                    output.logits, targets, valid, position_weight,
        ...
        return {
            ...
            "batch_size": min(int(batch_size), len(rows)) if batch_size is not None else len(rows),
    ...
    def fit_selector_module(
        ...
        for step in range(int(steps)):
            ...
            with self._autocast():
                numerator = None
                denominator = 0.0
                indices = self._batch_indices(len(hidden_rows), batch_size, step)
    ...
    def fit_joint(
        ...
        rows = list(records)
        if not rows:
            raise ValueError("joint training requires at least one trajectory record")
        if batch_size is not None and int(batch_size) <= 0:
            raise ValueError("batch_size must be positive or None")
        selector = selector.to(self.device)
        ...
        source_indices = [SourceNgramIndex(record.source_ids) for record in rows]
        histories = [
        ...
        for step in range(int(steps)):
            ...
            batch_anchor_indices = [
                record.anchors[0] if record.anchors else 0 for record in batch_rows
            ]
            masked, targets, valid = build_stage1_batch(
            ...
            with self._autocast():
                ...
                diffusion = diffusion_loss(output.logits, targets, valid)
                candidate_ids, candidate_logits = top_m_candidates(
                    output.logits, self.model.config.top_m
                )
                numerator = None
                denominator = 0.0
                for local_row, (hidden_row, ids_row, logits_row, targets_row, valid_row) in enumerate(zip(
                    output.hidden, candidate_ids, candidate_logits, targets, valid
                )):
                    row = indices[local_row]
                    selected = selector.select(
                        hidden_row, ids_row, logits_row, histories[row], source_indices[row],
