...
def test_syncspec_is_registered_in_dispatcher_and_uses_shared_config() -> None:
    ...
    wrapper = (ROOT / "scripts/runners/run_syncspec.sh").read_text(encoding="utf-8")
...
def test_syncspec_launcher_exposes_adaptive_budget_profiles() -> None:
    ...
    b200_runner = (ROOT / "scripts/runners/run_syncspec_b200_smoke.sh").read_text(encoding="utf-8")
...
def test_syncspec_cuda_smoke_has_explicit_cuda_guard() -> None:
    runner = (ROOT / "scripts/runners/run_syncspec_cuda_smoke.sh").read_text(encoding="utf-8")
...
def test_syncspec_cuda_smoke_probes_fast_infer_venv_runtime(tmp_path: Path) -> None:
    ...
    proc = subprocess.run(
        ["bash", "scripts/runners/run_syncspec_cuda_smoke.sh", str(tmp_path / "missing.env")],
...
def test_syncspec_cpu_smoke_covers_stage0_train_profile_and_infer() -> None:
    runner = (ROOT / "scripts/runners/run_syncspec_cpu_smoke.sh").read_text(encoding="utf-8")
