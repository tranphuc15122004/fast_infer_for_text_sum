...
#
# Options:
#   --baselines a,b,c      baseline có adapter representative (mặc định: 11 baseline)
...
REPRESENTATIVE_BASELINES="llmlingua fastkv gemfilter specprefill minference specextend eagle3 semantic_selection dflash longspec flexprefill"
...
apply_full_model_overrides() {
  local b="$1"
  if [[ "$MODE" != "full" ]]; then
    # T4 smoke: use smaller model pairs that fit 16GB and are cached locally.
...
for b in $BASELINES; do
  if [[ " $REPRESENTATIVE_BASELINES " != *" $b "* || \
        ( "$MODE" == "smoke" && " $T4_SMOKE_KERNEL_BASELINES " == *" $b "* ) ]]; then
    # baseline smoke/pipeline riêng: chạy 1 lần theo config smoke của nó
    # (kể cả longspec khi mode=smoke: kernel probe T4-safe, không tính metric)
    if [[ "$DRY_RUN" == "1" ]]; then
      echo
...
for b in $BASELINES; do
  case " $REPRESENTATIVE_BASELINES " in
    *" $b "*)
      if [[ "$MODE" == "smoke" && " $T4_SMOKE_KERNEL_BASELINES " == *" $b "* ]]; then
        continue
      fi
      EXPECTED_BASELINES="$EXPECTED_BASELINES $b" ;;
  esac
done
